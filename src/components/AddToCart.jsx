...
const Cart = () => {
    ...
    const handleRemoveFromCart = (productId) => {
        ...
        let productIndex = updatedCart.findIndex(item => item.product_id === productId);
        ...
        // Recalculate total cost after the update
        const total = updatedCart.reduce((acc, product) => acc + parseFloat(product.product_cost) * product.quantity, 0);
        setTotalCost(total);
    };
                    ...
                    {cart.map((product, index) => (
                        <div key={index} className="col-md-6 mb-4 d-flex justify-content-center">
                            <div className="card shadow-lg" style={{ borderRadius: '10px', borderColor: '#FFD700' }}>
                                ...
                                <div className="p-3">
                                    {/* Remove one item from Cart Button with Icon */}
                                    <button
                                        className="btn btn-danger w-100 mb-3"
                                        onClick={() => handleRemoveFromCart(product.product_id)}
                                    >
                                        <FaTrash className="mr-2" /> Remove one item
                                    </button>
